make_era5land_family_fixture <- function(path,
                                         dates = as.Date("2026-02-01") + 0:2,
                                         time_name = "time") {
  if (!requireNamespace("ncdf4", quietly=TRUE)) stop("ncdf4 is required")
  dates <- as.Date(dates)
  lon <- ncdf4::ncdim_def("longitude","degrees_east",c(-126.5,-126.0)); lat <- ncdf4::ncdim_def("latitude","degrees_north",c(42.5,43.0)); time <- ncdf4::ncdim_def(time_name,"hours since 2026-02-01 00:00:00",as.numeric(dates - as.Date("2026-02-01")) * 24,calendar="standard")
  ...
  for (n in names(vars)) { z <- array(specs[[n]]$base,dim=c(2,2,length(dates))); z[1,1,1] <- -9999; z[2,2,] <- z[2,2,] + seq(0,.2,length.out=length(dates)); ncdf4::ncvar_put(nc,vars[[n]],z) }
