...
request_inventory <- field(ans, "request_inventory", list())
  ...
  field(source,"raw_reused",field(manifest,"raw_reused",NA)),field(source,"archive_member_count",NA),field(source,"source_map_rows",NA),
   length(field(manifest,"requested_product_dates",character())),field(manifest,"daily_outputs_written",0),field(manifest,"daily_outputs_reused",0),length(field(manifest,"complete_iso_weeks_expected",character())),field(manifest,"weekly_outputs_expected",0),field(manifest,"weekly_outputs_written",0),field(manifest,"weekly_outputs_reused",0),field(manifest,"weekly_outputs_failed",0),paste(field(manifest,"failed_product_weeks",character()),collapse=","),
...
ok <- if (mode == "plan") identical(ans$status, "planned") else if (dry) ans$status %in% c("planned", "downloaded") else if (mode == "retrieve-requests") ans$status %in% c("retrieved", "retrieval_pending", "success", "success_noop") else if (mode == "stage-requests") ans$status == "staged" else if (mode %in% c("execute", "full")) ans$status %in% c("success", "success_noop", "execute_pending") else ans$status %in% c("success", "success_noop")
if(!ok) {
