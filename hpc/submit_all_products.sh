...
validation_job="$(sbatch --parsable --dependency="$aggregation_dependency" --job-name="cds_portfolio_validate" --output="$CDS_DATAGRAB_ROOT/logs/slurm/production/cds_portfolio_validate_%j.out" --error="$CDS_DATAGRAB_ROOT/logs/slurm/production/cds_portfolio_validate_%j.err" --export=ALL,REPO_DIR,PORTFOLIO_MANIFEST="$MANIFEST",CDS_DATAGRAB_R_LIB="$CDS_DATAGRAB_R_LIB",CHIME_EXECUTION_ID="$CHIME_EXECUTION_ID" "$REPO_DIR/hpc/run_portfolio_validate.slurm")"
