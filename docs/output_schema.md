...
New manifests also contain `chime_execution_id`, which is either `null` for
standalone cds-datagrab runs or an optional opaque external provenance
identifier supplied by CHIME. It does not affect processing, run identity,
validation, or portfolio status. `CHIME_EXECUTION_ID` is validated as a
non-empty value of at most 128 ASCII identifier characters with no control or
unsafe characters before it reaches Slurm or the manifest.
